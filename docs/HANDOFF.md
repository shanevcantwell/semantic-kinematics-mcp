...
**Written:** 2026-06-08  
**Updated:** 2026-06-11 — ADR-SKM-0008 and ADR-SKM-0009 promoted to Accepted; BulkEmbedder
merged to main (PR #12); ADR-SKM-0008 migration tracked in sk-mcp #11.  
...
- **Gap:** the **data pipeline is undocumented end-to-end** — BulkEmbedder is named (L357/369) but the flow (tvi adapters → `UnifiedMessage` → exporter → embedding_bridge → BulkEmbedder → null cache → axis alignment) is absent. **Stale:** tool list says "9 tools" incl. `model_load`/`model_unload` + StateManager (ADR-SKM-0009 deletes these); predates bearing/jolt/conditioned-atom entirely.
- **Move:** slim README to {what-it-is · quickstart · backends · links}; relocate tool-reference + math to ARCHITECTURE.md (or a TOOLS.md); add a data-pipeline section; refresh the tool set post-ADR-SKM-0009 + bearing. *(Candidate GitHub issue — not yet filed.)*
...
The three ADRs interlock in a specific order of dependency: ADR-SKM-0008 (unified
adapter) must land before ADR-SKM-0007's null cache can receive vault-produced
embeddings, and ADR-SKM-0009 (stateless MCP) depends on ADR-SKM-0008's per-call adapter
resolution. **ADR-SKM-0008 and ADR-SKM-0009 are now Accepted** (PR #10 promoted ADR-SKM-0009 on
2026-06-08; ADR-SKM-0008 followed on 2026-06-11, its `model_name` format settled
jointly with ADR-SKM-0009 Resolution 1). ADR-SKM-0007 remains Proposed pending real-corpus
...
|-----|---------------------|--------|------------|--------|
| [ADR-SKM-0007](ADRs/proposed/ADR-SKM-0007-referential-axis-alignment.md) | Add `analyze_axis_alignment` tool: project sentences onto an anchor-defined axis, z-scored against a corpus null | merged to main | — | Proposed |
| [ADR-SKM-0008](ADRs/proposed/ADR-SKM-0008-unified-embedding-adapter.md) | One `EmbeddingAdapter` ABC shared by sk-mcp and thought-vault; `BulkEmbedder` wraps any adapter for corpus-scale work | merged to main | — | **Accepted (2026-06-11)** |
| [ADR-SKM-0009](ADRs/proposed/ADR-SKM-0009-stateless-mcp-contract.md) | sk-mcp MCP tools become stateless (model selection per-call); `model_load`/`model_unload` removed; lifecycle moves to llauncher | merged to main (PR #10) | — | **Accepted (2026-06-08)** |

### ADR-SKM-0007 — Referential axis-alignment analysis
...
Open questions left by ADR-SKM-0007: which of the three readouts (position trace,
...
ADR numbering scheme (`ADR-SKM-0007` vs. `ADR-CORE-NNN`).

### ADR-SKM-0008 — Unified embedding adapter / BulkEmbedder
...
label), which allows vault-produced embeddings to key into ADR-SKM-0007's null cache.

~~Open questions left by ADR-SKM-0008~~ — resolved at acceptance (2026-06-11, see the
...
### ADR-SKM-0009 — Stateless MCP control-plane
...
~~Open questions left by ADR-SKM-0009~~ — resolved at acceptance (2026-06-08, PR #10,
...
multi-chunk ones (all stored embeddings unit-norm — load-bearing for ADR-SKM-0007
...
|-------|-------|------------------|
| [sk-mcp #2](https://github.com/shanevcantwell/semantic-kinematics-mcp/issues/2) | Implement ADR-SKM-0009: stateless MCP control-plane | sk-mcp MCP surface becomes reproducible and self-contained; prerequisite for clean multi-session use; now scoped against the Accepted ADR-SKM-0009 resolutions |
| [sk-mcp #3](https://github.com/shanevcantwell/semantic-kinematics-mcp/issues/3) | Run chat-log corpus through BulkEmbedder; validate ~45 min throughput | Validates BulkEmbedder on real data; produces embeddings that serve as ADR-SKM-0007 axis-alignment null. **Unblocked by PR #12** |
...
| [sk-mcp #11](https://github.com/shanevcantwell/semantic-kinematics-mcp/issues/11) | Implement ADR-SKM-0008: unified embedding adapter migration | Adapter generalization, `model_name` canonicalization (null-cache-invalidating), normalization audit, vault cutover — the gate between #3's embeddings and the ADR-SKM-0007 null cache |
...
| [llauncher #155](https://github.com/shanevcantwell/llauncher/issues/155) | Add vLLM/SentenceTransformers server type for non-GGUF embedding models | Lets llauncher own nv_embed (4096-d) lifecycle; prerequisite for ADR-SKM-0009's full stateless nv_embed path |

---

## 6. Open questions / unresolved

- **ADR numbering scheme.** The ADRs use project-local `ADR-SKM-0007/0008/0009` (formerly
  `ADR-001/002/003`, renumbered 2026-07-18) but an
...
- ~~**Canonical `model_name` format.**~~ Resolved 2026-06-11 (ADR-SKM-0008
  Resolution 1, jointly with ADR-SKM-0009 Resolution 1): llauncher's canonical
  string, e.g. `embeddinggemma-300M-F32`; transport metadata is adapter
  construction detail. Migration of legacy-keyed caches tracked in sk-mcp #11.

- ~~**Per-call parameter shape for stateless tools (ADR-SKM-0009).**~~ Resolved
  2026-06-08 (ADR-SKM-0009 Resolution 1): `model_name` + `base_url`, env fallback
  (`EMBEDDING_MODEL` / `EMBEDDING_SERVER_URL`); no backend enum in call identity.

- ~~**`StateManager` fate (ADR-SKM-0009).**~~ Resolved 2026-06-08 (ADR-SKM-0009
  Resolution 2): thin stateless resolver — class kept, retained fields, mutator,
  and cross-call cache removed.

- **Which readout leads `analyze_axis_alignment` (ADR-SKM-0007).** Three candidates:
...
- **Null-cache staleness beyond model-name match (ADR-SKM-0007).** Current guard
...
- **Normalization audit (ADR-SKM-0008).** The LM Studio adapter currently does not
  L2-normalize; embeddinggemma via llama-server behavior is not yet verified. ADR-SKM-0008
...
- **Sequencing: llauncher #155 vs. sk-mcp ADR-SKM-0009 cutover.** sk-mcp can go
...
- ~~**BulkEmbedder module location (ADR-SKM-0008).**~~ Resolved 2026-06-11 (ADR-SKM-0008
...
Done since the original index: ADR-SKM-0009 accepted (PR #10), ADR-SKM-0008 accepted,
BulkEmbedder merged with review fixes (PR #12), ADR-SKM-0008 migration issue filed
...
2. **Run the corpus through BulkEmbedder** (sk-mcp #3). Validates the engine on
   ...
   ~45 min target) and surfaces tuning needs before ADR-SKM-0008 migration work.
...
3. **ADR-SKM-0008 migration** (sk-mcp #11): generalize the OpenAI-compatible adapter;
...
5. **Build the ADR-SKM-0007 axis-alignment null** from the re-embedded corpus
   (`scripts/build_axis_null.py`); validate `analyze_axis_alignment` against
   real signal. This is also the natural point to resolve ADR-SKM-0007's remaining
...
6. **Implement ADR-SKM-0009** (sk-mcp #2) per the accepted resolutions: per-call
...
### §8 rev 2026-06-15 (b) — ADR-SKM-0002 tool contract; Spike A run; IMPLEMENTATION START POINT
**Canon now: ADR-SKM-0002** (`docs/ADRs/proposed/ADR-SKM-0002-bearing-analysis-tool-contract.md`, PROPOSED) on branch `docs/adr-skmcp-0002` (`c7f86d1`, **PR #27**). Operationalizes ADR-SKM-0001 (0001 = the math; 0002 = how it ships); extends the ADR-SKM-0007 build-validated-artifact + one-call-consume pattern (`build_axis_null.py` → `analyze_axis_alignment`) into the bearing regime.
...
- **Self-describing artifact = load-bearing object** (embedder identity, axis-validation verdict, null protocol + self-validation diagnostics, provenance). **Five cross-cutting rules as REFUSALS**: null-exemption typing · embedder-*validated* not just labeled · circularity guard · falsification-shaped confidence · target-state forms (post ADR-SKM-0009/#2 stateless per-call `model_name`+`base_url`, ADR-SKM-0008/#11 canonical `model_name`).
...
4. Build against **target forms** (ADR-SKM-0009 stateless, ADR-SKM-0008 canonical `model_name`) — do NOT build on `model_load`/`model_unload` or backend-prefixed names. Test coverage per the project rule.
   Open Qs (ADR-SKM-0008): null summary shape (empirical-quantile if non-Gaussian) · `cosine` exemption · status-tool-in-v1 · position-regime rename (`initialize_axis_analysis`/`run_axis_analysis`) · forge schema.
...
**Issue #25:** Spike A item = DONE-as-pipeline (flat fixture); items (1) Vogon run, (2) B-cluster mapping, (4) primitive build now subsumed under ADR-SKM-0008 implementation; (5) #14 defaults still deferred; (6) PR #26 merged + branches/worktree cleaned ✓.
**Git state (session close):** sk-mcp `main` @ `07803d1` (PRs #26 seam/nomic · #27 ADR-SKM-0008+handoff · #28 Spike A script · #29 gitignore `.claude/`); harness-tools `main` @ `f777d96` (#32 git-agent def refinements · #33 host-config canonical). Spike A script tracked at `scripts/spike_a_escalation_svd.py`.
...
**README accuracy pass:** corrected aspirational "stateless core" (it's stateful pre-ADR-SKM-0009) + inert `embed_text` model param; documented BulkEmbedder + `embed_corpus.py`.
...
**Bearing primitive (ADR-SKM-0001) confirmed NOT implemented** — only ADR-SKM-0007 axis-alignment exists. Seam now in place: `TrajectoryAnalyzer.analyze_embeddings(matrix)` / `analyze_segments(list)` (no spaCy split) — Spike B plugs in here.
...
> narrowly — it treated ADR-SKM-0007's `analyze_axis_alignment` tool as the
> destination. **ADR-SKM-0001** (directional projection primitive, now promoted
...
|---|---|---|---|---|
| **Position / rhythm** | "where does this sit / how does it drift?" | sentence (point) | **corpus-null** — where real text *sits* on the axis | ADR-SKM-0007 axis-alignment; velocity/curvature (inherited) |
| **Bearing / motion** | "which way did this *move* vs a named axis?" | **displacement** (+ anchor pair) | **measured-displacement-null** — what random *motion* looks like, given anisotropy | ADR-SKM-0001 projection primitive |
...
Diagnosis from ADR-SKM-0001: the reflexive angle measures (drift cosine,
...
- **Dest 1 — map the thought-vault corpus** (position regime): drift/position
  over the personal corpus; ADR-SKM-0007 corpus-null; **needs the full vault embedded
  at scale** → infra-heavy (#11, #16, #17, #20, resolver).
- **Dest 2 — measure behavioral axes directionally** (bearing regime; escalation
  → semantic-forge judge calibration): ADR-SKM-0001 projection; the axis comes
...
SHARED INFRA BASE:  resolver(#2/#14/#15) · #16 · #17 · #20(Spike C) · #11
  ├─ Dest 1 (vault mapping):    base → corpus embed at scale → ADR-SKM-0007 corpus-null
  │                             → drift/position analysis
  └─ Dest 2 (behavioral axis):  [Spike A] escalation-axis SVD → [Spike B] projection
                                + measured-displacement-null → ADR-SKM-0001 implemented
...
protocol; go/no-go on the primitive → whether ADR-SKM-0001 graduates from
...
**Resolved 2026-07-18.** The `ADR-00N` vs `ADR-SKMCP-NNNN` split (ADR-SKM-0001
previously kept its `SKMCP-NNNN` identity for cross-repo refs rather than
renumbering to local `ADR-004`) is now reconciled: both series are unified
under the registered code `SKM` (4-digit padding). The old bare trio
(`ADR-001`/`002`/`003`) is renumbered to `ADR-SKM-0007`/`0008`/`0009`; the
former `ADR-SKMCP-000N` files are `ADR-SKM-000N`. See the migration PR for
