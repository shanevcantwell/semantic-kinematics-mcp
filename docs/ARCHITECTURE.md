...
- **BulkEmbedder (`embeddings/bulk.py`):** wraps any `EmbeddingAdapter`; adds checkpoint/resume, token-aware batching, sub-chunking with vector averaging, and backoff retries. Driven by `scripts/embed_corpus.py` (see README → *Bulk Embedding a Corpus* for the CLI). It is a data-plane application on the shared substrate — it invokes no `mcp/commands/` primitive and crosses no contract boundary. If a bulk job needs analysis results, it re-enters through the MCP contract like any other consumer.
